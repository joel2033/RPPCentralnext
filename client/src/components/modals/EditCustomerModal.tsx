...
export default function EditCustomerModal({ onClose, customer, customerId }: EditCustomerModalProps) {
  ...
  useEffect(() => {
    if (customer) {
      ...
      let parsedTeamMembers: TeamMember[] = [];
      if (customer.teamMembers) {
        try {
          const members = typeof customer.teamMembers === 'string' 
            ? JSON.parse(customer.teamMembers) 
            : customer.teamMembers;
          parsedTeamMembers = members.map((member: any, index: number) => ({
            id: `member-${index}`,
            name: member.name || "",
            email: member.email || "",
            role: member.role || ""
  ...
  const updateCustomerMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await apiRequest(`/api/customers/${customerId}`, "PATCH", data);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 modal-backdrop z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-bold text-gray-900" data-testid="text-modal-title">Edit Customer</h2>
            <p className="text-sm text-gray-500 mt-1">
              Update customer details, billing preferences, and team members.
            </p>
          </div>
          <button 
            onClick={onClose} 
