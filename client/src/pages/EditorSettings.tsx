...
import { User, MapPin, Phone, Mail, Camera, DollarSign, Clock, Settings, Handshake, Calendar, Users, Building2, Plus, Edit2, Trash2, Package, Bell } from "lucide-react";
...
export default function EditorSettings() {
  ...
  // Handle initial hash navigation and legacy support
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const hash = window.location.hash.replace('#', '');
      if (hash === 'services-pricing') {
        // Legacy support: redirect services-pricing hash to services tab
        setActiveTab('services');
        window.location.hash = 'services';
      } else if (hash && ['profile', 'business', 'services', 'partnerships'].includes(hash) && hash !== activeTab) {
        setActiveTab(hash);
      }
    }
  ...
  const getInitialTab = () => {
    if (typeof window !== 'undefined') {
      const hash = window.location.hash.replace('#', '');
      if (hash && ['profile', 'business', 'services', 'partnerships'].includes(hash)) {
        return hash;
      }
    }
    return 'profile';
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      placeholder="Tell partners about your experience and expertise..."
            ...
            <TabsContent value="business" className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Configure your job management, working hours, and notifications
                  </p>
                </CardHeader>
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Button onClick={handleCreateCategory} size="sm" data-testid="button-create-category" className="!bg-blue-600 hover:!bg-blue-700">
                  ...
                  {servicesLoading ? (
                    ...
                    <div className="space-y-4">
                      {editorServices.map((service) => {
                        ...
                        return (
                          <div key={service.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            <div>
                              <Label className="text-xs text-gray-500">Service Name</Label>
                              <p className="font-medium text-gray-900 mt-1" data-testid={`text-service-name-${service.id}`}>
                                {service.name}
                              </p>
                              {category && (
                            ...
                            <div>
                              <Label className="text-xs text-gray-500">Price (per {service.pricePer || 'image'})</Label>
                              <p className="font-medium text-gray-900 mt-1" data-testid={`text-service-price-${service.id}`}>
                                ${parseFloat(service.basePrice).toFixed(2)}
                              </p>
                            </div>
                            <div>
                              <Label className="text-xs text-gray-500">Turnaround</Label>
                              <p className="text-gray-700 mt-1" data-testid={`text-service-turnaround-${service.id}`}>
                            ...
                            <div className="flex items-center justify-end gap-2">
                              <Badge 
                                ...
                                className={service.isActive !== false ? 'bg-[rgba(141,214,81,1)] hover:bg-[rgba(141,214,81,0.9)]' : undefined}
                              >
          ...
          <Card className="sticky top-24">
            ...
            <CardContent className="space-y-4">
              <div className="text-center">
                ...
                <h3 className="font-semibold text-gray-900 text-sm">{profileData.businessName}</h3>
              ...
              <div className="space-y-3 text-sm pt-4 border-t">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs">{profileData.contactPhone}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <Mail className="w-4 h-4 text-gray-400" />
                  <span className="text-xs truncate">{profileData.contactEmail}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <Clock className="w-4 h-4 text-gray-400" />
                  <span className="text-xs">{businessSettings.averageTurnaroundHours}h avg turnaround</span>
                </div>
              </div>

              <div className="pt-4 border-t">
                <h4 className="font-medium text-gray-900 mb-2 text-sm">Active Services</h4>
                <div className="space-y-2">
                  {editorServices.filter(service => service.isActive !== false).slice(0, 5).map((service) => (
                    <div key={service.id} className="flex items-center justify-between text-xs">
                      ...
                      <Badge variant="outline" className="ml-2 text-xs">${parseFloat(service.basePrice).toFixed(2)}</Badge>
                    </div>
                  ))}
